...
class trx_addons_demo_data_importer
{
            ...
            <div class="trx_importer_section">
                ...
                <form id="trx_importer_form">
                    ...
                    <?php if (count($this->options['files']) > 1) { ?>
                        ...
                        <div class="trx_importer_demo_type">
                            ...
                            foreach ($this->options['files'] as $k => $v) {
                                ?><label><input
                                type="radio"<?php if ($this->options['demo_type'] == $k) echo ' checked="checked"'; ?>
                                value="<?php echo esc_attr($k); ?>"
                                name="demo_type" /><?php echo esc_html($v['title']); ?></label><?php
                            }
                            ?>
                        </div>
                    <?php } ?>
                    ...
                    <div class="trx_importer_demo_set">
                        <label><input
                                    type="radio"<?php if ($this->options['demo_set'] == 'part') echo ' checked="checked"'; ?>
                                    value="part"
                                    name="demo_set"/><?php esc_html_e('Only pages, forms and sliders', 'trx_addons'); ?>
                        </label>
                        ...
                        <label><input
                                    type="radio"<?php if ($this->options['demo_set'] == 'full') echo ' checked="checked"'; ?>
                                    value="full"
                                    name="demo_set"/><?php esc_html_e('Whole demo-site content', 'trx_addons'); ?>
                        </label>
                            ...
                            <div class="trx_importer_part_pages<?php if ($this->options['demo_set'] == 'full') echo ' trx_importer_hidden"'; ?>">
                                ...
                                if (is_array($pages)) {
                                    foreach ($pages as $id => $title) {
                                        ?>
                                        <label>
                                            <input class="trx_importer_pages" type="checkbox"
                                                   value="<?php echo esc_attr($id); ?>"
                                                   name="import_pages_<?php echo esc_attr($id); ?>"
                                                   id="import_pages_<?php echo esc_attr($id); ?>"/>
                ...
                <div id="trx_importer_progress"
                     class="notice notice-info style_<?php echo esc_attr($this->options['demo_style']); ?>">
                    ...
                    <h4 class="trx_importer_progress_complete"><?php esc_html_e('Congratulations! Data import complete!', 'trx_addons'); ?>
                        <a href="<?php echo esc_url(home_url('/')); ?>"><?php esc_html_e('View site', 'trx_addons'); ?></a>
                    </h4>
                </div>
                ...
                <form id="trx_exporter_form" action="#" method="post">
                    ...
                    if (isset($_POST['exporter_action'])) {
                        ...
                        if (false && count($this->options['files']) > 1) {
                            ...
                            ?><p><b><?php esc_html_e('Select the demo type to be exported', 'trx_addons'); ?></b>
                            </p><?php
                            ...
                            foreach ($this->options['files'] as $k => $v) {
                                if (!empty($v['file_with_posts'])) {
                                    ...
                                    <label><input
                                                type="radio"<?php if ($this->options['demo_type'] == $k) echo ' checked="checked"'; ?>
                                                value="<?php echo esc_attr($k); ?>"
                                                name="demo_type"/><?php echo esc_html($v['title']); ?></label>
                                    <?php
                                }
                            }
                        }
    ...
    function show_exporter_fields($args = array())
    {
        ...
        <tr>
            ...
            <td>
                <a download="<?php echo esc_attr(!empty($args['download']) ? $args['download'] : $args['slug'] . '.txt'); ?>"
                   href="<?php echo esc_url(trx_addons_get_file_url(TRX_ADDONS_PLUGIN_IMPORTER . 'export/' . trim($args['slug']) . '.txt')); ?>"><?php esc_html_e('Download', 'trx_addons'); ?></a>
            </td>
        </tr>
        <?php
    }
    ...
    function import_thumbnails()
    {
        ...
        foreach ($files as $id => $file) {
            ...
            // Break import after timeout or if attachments loading from parts - to show percent loading after each part
            if (time() - $this->start_time >= $this->max_time || ++$processed >= 5)
                break;
        }
