...
//            echo trx_addons_sc_stack_check('show_layout');
    ...
    function trx_addons_cpt_layouts_elm_layout_content($content, $post_id = 0, $force_styles = false)
    {
        ...
        if (!empty($layout)) {
            ...
            if (trx_addons_exists_elementor()) {
                ...
                $cur_page_built_with_elementor = \Elementor\Plugin::instance()->db->is_built_with_elementor($post_id);
                if ($cur_page_built_with_elementor) {
                    ...
                    if (trx_addons_sc_stack_check('show_layout')) {
                        ...
                        $post_content = \Elementor\Plugin::instance()->frontend->get_builder_content($post_id, $force_styles || (empty($TRX_ADDONS_STORAGE['cur_page_built_with_elementor']) && empty($styles_included[$post_id])));
