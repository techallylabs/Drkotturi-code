...
if (!function_exists('trx_addons_cpt_competitions_init')) {
	...
	function trx_addons_cpt_competitions_init() {
		...
		$TRX_ADDONS_STORAGE['post_types'][] = TRX_ADDONS_CPT_COMPETITIONS_PT;
		$TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_COMPETITIONS_PT] = array(
			"date_start" => array(
...
if ( !function_exists( 'trx_addons_cpt_competitions_post_type_taxonomy' ) ) {
	add_filter( 'trx_addons_filter_post_type_taxonomy',	'trx_addons_cpt_competitions_post_type_taxonomy', 10, 2 );
	function trx_addons_cpt_competitions_post_type_taxonomy($tax='', $post_type='') {
		if ( $post_type == TRX_ADDONS_CPT_COMPETITIONS_PT )
...
if ( !function_exists( 'trx_addons_cpt_competitions_get_blog_all_posts_link' ) ) {
	add_filter('trx_addons_filter_get_blog_all_posts_link', 'trx_addons_cpt_competitions_get_blog_all_posts_link', 10, 2);
	function trx_addons_cpt_competitions_get_blog_all_posts_link($link='', $args=array()) {
		if ($link=='') {
			if (trx_addons_is_competitions_page() && !is_post_type_archive(TRX_ADDONS_CPT_COMPETITIONS_PT))
...
if ( !function_exists( 'trx_addons_cpt_competitions_get_blog_title' ) ) {
	add_filter( 'trx_addons_filter_get_blog_title', 'trx_addons_cpt_competitions_get_blog_title');
	function trx_addons_cpt_competitions_get_blog_title($title='') {
		if ( is_post_type_archive(TRX_ADDONS_CPT_COMPETITIONS_PT) )
			$title = esc_html__('All Competitions', 'trx_addons');
		return $title;
	}
}
