...
if (!function_exists('trx_addons_cpt_agents_init')) {
	...
	function trx_addons_cpt_agents_init() {
		...
		$TRX_ADDONS_STORAGE['post_types'][] = TRX_ADDONS_CPT_AGENTS_PT;
		$TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_AGENTS_PT] = array(
			"description" => array(
...
if ( !function_exists( 'trx_addons_cpt_agents_post_type_taxonomy' ) ) {
	add_filter( 'trx_addons_filter_post_type_taxonomy',	'trx_addons_cpt_agents_post_type_taxonomy', 10, 2 );
	function trx_addons_cpt_agents_post_type_taxonomy($tax='', $post_type='') {
		if ( defined('TRX_ADDONS_CPT_AGENTS_PT') && $post_type == TRX_ADDONS_CPT_AGENTS_PT )
			$tax = TRX_ADDONS_CPT_AGENTS_TAXONOMY;
		return $tax;
	}
}
...
if ( !function_exists( 'trx_addons_cpt_agents_get_blog_all_posts_link' ) ) {
	add_filter('trx_addons_filter_get_blog_all_posts_link', 'trx_addons_cpt_agents_get_blog_all_posts_link', 10, 2);
	function trx_addons_cpt_agents_get_blog_all_posts_link($link='', $args=array()) {
		if ($link=='') {
			if (trx_addons_is_agents_page() && (!is_post_type_archive(TRX_ADDONS_CPT_AGENTS_PT) || is_tax(TRX_ADDONS_CPT_AGENTS_TAXONOMY)))
				$link = '<a href="'.esc_url(get_post_type_archive_link( TRX_ADDONS_CPT_AGENTS_PT )).'">'
...
if ( !function_exists( 'trx_addons_cpt_agents_get_blog_title' ) ) {
	add_filter( 'trx_addons_filter_get_blog_title', 'trx_addons_cpt_agents_get_blog_title');
	function trx_addons_cpt_agents_get_blog_title($title='') {
		if ( defined('TRX_ADDONS_CPT_AGENTS_PT') ) {
