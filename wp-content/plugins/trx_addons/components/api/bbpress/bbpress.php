...
if ( !function_exists( 'trx_addons_bbpress_get_blog_all_posts_link' ) ) {
	...
	function trx_addons_bbpress_get_blog_all_posts_link($link='', $args=array()) {
		if ($link=='' && trx_addons_is_bbpress_page()) {
			...
			$link = '<a href="'.esc_url($url).'">'.esc_html__('Forums', 'trx_addons').'</a>';
		}
		return $link;
	}
}
	...
	function trx_addons_bbpress_importer_show_params($importer) {
		if ( trx_addons_exists_bbpress() && in_array('bbpress', $importer->options['required_plugins']) ) {
			$importer->show_importer_params(array(
				...
				'description' => wp_kses_data('<b>Attention! BuddyPress and bbPress increases the memory consumption at 60-80MB</b> depending on the configuration of the plugins. So if you have problems with installing the demo-data due to lack of memory - deactivate BuddyPress and bbPress plugins and then retry the installation of the demo-data.', 'trx_addons'),
				'part' => 0
			));
		}
	}
...
if ( !function_exists( 'trx_addons_bbpress_importer_export' ) ) {
	...
	function trx_addons_bbpress_importer_export($importer) {
		if ( trx_addons_exists_bbpress() && in_array('bbpress', $importer->options['required_plugins']) ) {
			trx_addons_fpc(trx_addons_get_file_dir(TRX_ADDONS_PLUGIN_IMPORTER . 'export/bbpress.txt'), serialize( array(
