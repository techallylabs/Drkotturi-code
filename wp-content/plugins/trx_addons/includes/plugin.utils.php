...
if (!function_exists('trx_addons_array_insert_before')) {
	function trx_addons_array_insert_before(&$haystack, $needle, $stuff): int
    {
		if (! is_array($haystack) ) return -1;
...
if ( ! function_exists('trx_addons_url_replace') ) {
    function trx_addons_url_replace($from, $to, $str) {
        ...
        return trx_addons_str_replace(array(
            $from,
            str_replace('/', '\\/', $from),
            trx_addons_remove_protocol($from),
            str_replace('/', '\\/', trx_addons_remove_protocol($from)),
            trx_addons_remove_protocol($from, true),
            str_replace('/', '\\/', trx_addons_remove_protocol($from, true))
        ),
            array(
                $to,
                str_replace('/', '\\/', $to),
                trx_addons_remove_protocol($to),
                str_replace('/', '\\/', trx_addons_remove_protocol($to)),
                trx_addons_remove_protocol($to, true),
                str_replace('/', '\\/', trx_addons_remove_protocol($to, true))
            ),
            $str);
    }
}
