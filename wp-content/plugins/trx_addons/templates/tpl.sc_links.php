...
if (!empty($args['link_image']) && ($args['link_image'] = trx_addons_get_attachment_url($args['link_image'], trx_addons_get_thumb_size('medium')))!='') {
	...
	echo str_replace('sc_item_button',
					'sc_item_button sc_item_button_'.esc_attr($args['link_style']).' '.esc_attr($sc).'_button'.esc_attr($align),
					trx_addons_sc_button(apply_filters('trx_addons_filter_sc_item_button_args', array(
