...
class Plugin {
    ...
    const AJAX_NONCE     = Plugin::PREFIX . '-ajax';
    const AJAX_ARG       = 'security';

    const ASSETS_DIR    = 'Assets';
    const RESOURCES_DIR = 'Resources';
    const TEMPLATES_DIR = 'Templates';
    const SVG_DIR       = Plugin::ASSETS_DIR . '/svg';
    ...
    public static function listJsParams() {
        return [
            ...
            'ajaxNonce'         => wp_create_nonce(Plugin::AJAX_NONCE),
            'ajaxArg'           => Plugin::AJAX_ARG,
            'pluginPrefix'      => Plugin::PREFIX,
            'blogId'            => get_current_blog_id(),
            'isMultiSite'       => is_multisite(),
            'locale'            => get_locale(),
            'showModal'         => UserTypeHelper::showModal(),
        ];
    }
    ...
    public static function enqueueStyle( $file = '', $dependencies = [] ) {
        ...
        $handle = sanitize_title_with_dashes(Plugin::PREFIX . '-' . str_replace('.min.', '.', $file));
        wp_enqueue_style($handle, Plugin::getAssetsUrl('css') . $file, $dependencies, filemtime(Plugin::getAssetsDir('css') . $file));
        return $handle;
    }
    ...
    public static function enqueueScript( $file = '', $dependencies = [], $in_footer = false ) {
        ...
        $handle = sanitize_title_with_dashes(Plugin::PREFIX . '-' . str_replace('.min.', '.', $file));
        wp_enqueue_script($handle, Plugin::getAssetsUrl('js') . $file, $dependencies, filemtime(Plugin::getAssetsDir('js') . $file), $in_footer);
        return $handle;
    }
