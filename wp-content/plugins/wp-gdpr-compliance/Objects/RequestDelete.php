...
class RequestDelete extends AbstractRequest {
    ...
    public function getData( bool $new = false ): array {
        return array_merge(parent::getData($new), [
    ...
    public static function getOldRequests( string $limit = '-24 hours', $anonymous = NULL ): array {
        ...
        $filters = [
            static::KEY_CREATED => [ 'value' => $date->format('Y-m-d H:i:s'), 'compare' => '<=' ],
        ];
        if( !is_null($anonymous) ) $filters[ static::KEY_IP_ADDRESS ] = [ 'value' => Anonymous::getIpAddress(), 'compare' => $anonymous ? '=' : '!=' ];
    ...
    public static function getByTypeAndDataIdAndAccessId( string $type = '', int $data_id = 0, int $access_id = 0 ) {
        ...
        if( $result !== NULL ) return new static($result[ static::KEY_ID ]);
    ...
    public static function getByAccessId( int $access_id = 0 ): array {
        $list = [];
        if( empty($access_id) ) return $list;
    ...
    public static function getCountByAccessId( int $access_id = 0, bool $anonymous = true ): int {
        if( empty($access_id) ) return false;
        ...
        $query .= " AND `" . static::KEY_ACCESS_ID . "` = %d";
        $query .= sprintf(" AND `" . static::KEY_SITE_ID . "` = %d", get_current_blog_id());
        if( $anonymous === false ) $query .= " AND `" . static::KEY_IP_ADDRESS . "` != '" . Anonymous::getIpAddress() . "'";
    ...
    public static function anonymizeByAccessId( int $access_id = 0 ): array {
        ...
        foreach( $list as $object ) {
            ...
            $output['processed'][ $request_id ] = sprintf(__('%1s #%2s with email address %3s.', 'wp-gdpr-compliance'), $object->getNiceTypeLabel(), $object->getDataId(), $email);
        }
    ...
    public static function sendNotification( RequestAccess $object, array $data = [] ) {
        ...
        if( !empty($data['error']) && empty($data['processed']) ) return false;
        if( empty($data['processed']) ) return false;
        ...
        $site_link = Elements::getLink($site_url, $site_name, [ 'target' => '_blank' ]);
