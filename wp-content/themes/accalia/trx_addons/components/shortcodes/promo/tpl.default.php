...
if (strpos($args['image2'], ',')!==false)
    $args['image2'] = explode(',', $args['image2']);
else
    $args['image2'] = trx_addons_get_attachment_url($args['image2'], 'full');
	...
	if (!empty($args['title']) || !empty($args['subtitle']) || !empty($args['description']) || !empty($args['content']) 
		...
		?><div class="sc_promo_text<?php
			...
			<div class="sc_promo_text_inner<?php 
				...
				?>" style="<?php echo esc_attr($css_text_inner); echo esc_attr((!empty($args['image2']) && !is_array($args['image2']) ? ' background-image:url(' . esc_url($args['image2']) . ');' : ''));?>"><?php
				if (!empty($args['icon'])) {
