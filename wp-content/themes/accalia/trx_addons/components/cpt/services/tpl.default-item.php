	...
	<div class="sc_services_item_info">
		<div class="sc_services_item_header">
            <div class="sc_services_item_subtitle">
                <?php $terms = get_the_terms(get_the_ID(), TRX_ADDONS_CPT_SERVICES_TAXONOMY);
                if ( !empty( $terms ) ) {
                    $output = '';
                    foreach( $terms as $term ) {
                        $output .= ('<a href="' . esc_url( get_term_link( $term->term_id, TRX_ADDONS_CPT_SERVICES_TAXONOMY ) ) . '"'
                            . ' title="' . sprintf( esc_attr__( 'View all posts in %s', 'accalia' ), $term->name ) . '"'
                            . '>');
                        $output .= ($term->count);
                        $output .= (_n(' Procedure', ' Procedures', $term->count, 'accalia'));
                        $output .= '</a> ';
                    }
                }
			...
			<h5 class="sc_services_item_title"><a href="<?php echo esc_url($link); ?>"><?php the_title(); ?></a></h5>
		</div>
		<?php if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) { ?>
			<div class="sc_services_item_content"><?php the_excerpt(); ?></div>
			<div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="sc_button"><?php esc_html_e('Learn more', 'accalia'); ?></a></div>
		<?php } ?>
	</div>
</div>
<?php
