...
if (!function_exists('accalia_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'accalia_hovers_theme_setup3', 3 );
	function accalia_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		accalia_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'accalia'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'accalia') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'accalia'),
					'slide_left'	=> esc_html__('Slide from Left',	'accalia'),
					'slide_right'	=> esc_html__('Slide from Right',	'accalia'),
					'slide_top'		=> esc_html__('Slide from Top',		'accalia'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'accalia'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'accalia'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'accalia') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'accalia'),
					'icon'	=> esc_html__('Icon',	'accalia'),
					'icons'	=> esc_html__('Icons',	'accalia'),
					'zoom'	=> esc_html__('Zoom',	'accalia'),
					'fade'	=> esc_html__('Fade',	'accalia'),
					'slide'	=> esc_html__('Slide',	'accalia'),
					'pull'	=> esc_html__('Pull',	'accalia'),
					'border'=> esc_html__('Border',	'accalia')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('accalia_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'accalia_hovers_theme_setup9', 9 );
	function accalia_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'accalia_hovers_frontend_scripts', 1010 );
		add_filter( 'accalia_filter_localize_script','accalia_hovers_localize_script' );
		add_filter( 'accalia_filter_merge_scripts',	'accalia_hovers_merge_scripts' );
		add_filter( 'accalia_filter_merge_styles',	'accalia_hovers_merge_styles' );
		add_filter( 'accalia_filter_get_css', 		'accalia_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'accalia_hovers_frontend_scripts' ) ) {
	function accalia_hovers_frontend_scripts() {
		if ( accalia_is_on(accalia_get_theme_option('debug_mode')) && accalia_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'accalia-hovers', accalia_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( accalia_is_on(accalia_get_theme_option('debug_mode')) && accalia_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'accalia-hovers',  accalia_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('accalia_hovers_merge_scripts')) {
	function accalia_hovers_merge_scripts($list) {
...
if (!function_exists('accalia_hovers_merge_styles')) {
	function accalia_hovers_merge_styles($list) {
...
if (!function_exists('accalia_hovers_localize_script')) {
	function accalia_hovers_localize_script($arr) {
		$arr['button_hover'] = accalia_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('accalia_hovers_add_icons') ) {
	function accalia_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'accalia') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'accalia');
						else
							esc_html_e('Details', 'accalia');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'accalia_hovers_get_css' ) ) {
	function accalia_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
li.active .sc_button_hover_slide_left {		background-position: left bottom !important; color: {$colors['text_link3']} !important; }
